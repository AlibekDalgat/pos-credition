...
func (listPostgres *TodoListPostgres) GetAll(userId int) ([]todo.TodoList, error) {
	...
	err := listPostgres.db.Select(&lists, query, userId)
	return lists, err
}

func (listPostgres *TodoListPostgres) GetById(userId, id int) (todo.TodoList, error) {
	...
	err := listPostgres.db.Get(&list, query, userId, id)
	return list, err
}

func (listPostgres *TodoListPostgres) DeleteById(userId, id int) error {
	...
	_, err := listPostgres.db.Exec(query, userId, id)
	return err
}

func (listPostgres *TodoListPostgres) UpdateById(userId, id int, input todo.UpdateListInput) error {
	...
	_, err := listPostgres.db.Exec(query, args...)
	return err
}
